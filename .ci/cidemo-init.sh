...
snmp_plugin=$(git diff --name-only remotes/origin/$ghprbTargetBranch | grep snmp_receiver_plugin |grep -v .gitmodules |wc -l)
...
elif [ $snmp_plugin -eq $total ] && [ $total -ne 0 ]; then
    ln -snf ../plugins/snmp_plugin/.ci/ci_matrix.yaml matrix_job_ci.yaml
else
