        ...
        for root, _, files in os.walk(self.dir_path):
            for file_name in files:
                ...
                is_logs_with_dir_flag = (
                    last_dir_name in logs_with_dirs
                    and file_name in logs_with_dirs[last_dir_name]
                )
                if (
                    file_name in files_to_extract
                    or last_dir_name in directories_to_extract
                    or is_logs_with_dir_flag
                ):
                    src_file_path = os.path.join(root, file_name)
                    new_file_name = (
                        f"{last_dir_name}_{file_name}"
