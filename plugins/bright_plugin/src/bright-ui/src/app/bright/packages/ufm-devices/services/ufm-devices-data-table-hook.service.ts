...
import {BrightConstants} from "../../bright/constants/bright.constants";
  ...
  public appendBrightData(ufmDevices: Array<any>): Observable<any> {
    return new Observable<any>((observable) => {
      this.brightBackendService.getBrightConf().subscribe({
        next: (conf) => {
          if (conf[BrightConstants.brightConfKeys.brightConfig]
            [BrightConstants.brightConfKeys.status][BrightConstants.brightConfKeys.status] != BrightConstants.brightStatusValues.disabled) {
