...
async def receive_and_generate_client_certificate(recv_tag, file_path,
    ...
    # save and generate
    try:
        sertificate_dir = rdma_rest_config.get("Server",
                                               "certificate_file_location_dir",
                                     fallback=DEFAULT_CLIENT_CERT_LOCATION_DIR)
        if not os.path.exists(sertificate_dir):  # create if not exist
            os.makedirs(sertificate_dir)
        receive_location_file = "/".join([sertificate_dir,
                                          file_path.split('/')[-1]])
...
async def send_rest_request_client_certificate(real_url, action, payload,
    ...
    logging.debug("real_url %s, action %s, payload %s, sert file path %s, key file path %s" %
                    (real_url, action, payload, sert_file_path, key_file_path))
    if payload and payload != 'None':
        send_payload = json.loads(payload)
    else:
        send_payload = None
    cert = (sert_file_path, key_file_path)
...
async def handle_rest_request(end_point, recv_tag, send_tag, generate_cs=True):
    ...
    action_type, action, url, payload, username, password, host, client_sertificate  = \
    ...
    if action_type != ActionType.FILE_TRANSFER.value:  # REST REQUEST- RESPOND
        ...
        logging.debug("Server: Received: action %s, url %s, payload %s" %
                      (action, url, payload))
        # now if client_sertificate has value - need to receive it and to generate
        # certificate files
        if client_sertificate and client_sertificate != "None":
            sert_file_name = rdma_rest_config.get("Server", "client_pem_cert",
            ...
            sert_file_path = "/".join([sertificate_dir, sert_file_name])
            key_file_path = "/".join([sertificate_dir,key_file_name])
            # generate
            if generate_cs:
                client_sert_status = await receive_and_generate_client_certificate(
                                                      recv_tag, client_sertificate,
                                                      sert_file_path, key_file_path)
                if not client_sert_status:
...
def dump_pem_cert_and_key_from_pfx(pfx_file, pfx_data, sert_file_path,
    ...
    try:
        ...
        dump_pem_cert(sert_file_path, cert_obj)
    except Exception as e:
        logging.error("Server: Cannot convert the PFX file %s to PEM cert and key: %s" %
                                          (os.path.basename(pfx_file), str(e)))

def convert_pfx_cert_to_pem_cert_and_key(pfx_cert, sert_file_path, key_file_path):
    try:
        with open(pfx_cert, "rb") as f:
            cert_data = f.read()
        dump_pem_cert_and_key_from_pfx(pfx_cert, cert_data, sert_file_path,
                                       key_file_path, pfx_password='')
    except IOError as e:
