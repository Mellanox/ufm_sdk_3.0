...
# predefined params
BASE_ENVIRON_NAME = "UFM_PLUGIN_SYSINFO_{}"
...
def make_request(host_ip, request, auth_type, auth, payload, headers, cert):
    request_string = "https://{}/ufmRest{}/plugin/sysinfo/{}".format(host_ip, get_rest_version(auth_type), request)

    verify = True if cert else False
    if request in [SWITCHS, REPORTS, VERSION, HELP] or re.match(REPORT_ID_PATTERN, request):
...
def parse_args():
    parser = argparse.ArgumentParser(description='Sysinfo REST API provider',
                                     formatter_class=argparse.RawDescriptionHelpFormatter,
                                     epilog=textwrap.dedent('''\
                                              use following environment variables to set arguments permanently:
                                                  UFM_PLUGIN_SYSINFO_AUTH_TYPE
                                                  UFM_PLUGIN_SYSINFO_HOST
                                                  UFM_PLUGIN_SYSINFO_USER
                                                  UFM_PLUGIN_SYSINFO_PASSWORD
                                                  UFM_PLUGIN_SYSINFO_PRIVATE_KEY
                                                  UFM_PLUGIN_SYSINFO_CLIENT_CERTIFICATE
                                                  UFM_PLUGIN_SYSINFO_AUTHENTICATION_TOKEN
                                                e.g., "export UFM_PLUGIN_SYSINFO_AUTH_TYPE=client_cert"
                                              ''')
    )
    parser.add_argument('request', type=str, help='Request to complete',
                        choices=[SWITCHS, REPORTS, UPLOAD, COMPARE, DELETE, CANCEL, VERSION, HELP])
    parser.add_argument('-ip', '--host', type=str, help='Host IP address where SYSINFO is running')
...
def parse_upload_files(files):
    data = []
    for sysinfo in files.split(","):
        try:
            (file_path, file_type) = sysinfo.split("|")
        except ValueError:
...
def parse_delete_files(files):
    data = []
    for sysinfo in files.split(","):
        data.append({"file_name": "{}".format(sysinfo)})
    return data
