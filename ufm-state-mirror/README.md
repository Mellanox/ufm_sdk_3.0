...
- **Classifier** (supplied by the consumer at `CLASSIFIER_PATH`; see
  ...
  files are durable and how (handler, storage key, first-install baseline).
  Validated and fails closed at load.
...
- When `restore` overwrites an existing file it **preserves the original
  owner/group/mode**: the consumer seeds each file with its correct identity
  before restore, so the (typically root-run) restore re-applies the captured
  `uid/gid/mode` after its atomic replace. `chown` is best-effort and only runs
  when effectively root; a brand-new file is left to the consumer to own.
