...
class TestQueueBounds:
    ...
    def test_delete_queue_drops_oldest_when_full(self, fake_redis, tmp_path):
        ...
        assert len(m._deletes) == 3
        assert m.state.dropped_events_total == 7
        assert m._delete_reconcile_needed is True
        # The newest deletes are the ones retained (oldest dropped).
        assert m._deletes[-1][1].endswith("f9")


class TestDeleteReconcile:
    def test_full_scan_reconciles_dropped_directory_delete(self, fake_redis, tmp_path):
        root = tmp_path / "sites"
        root.mkdir()
        ...
        # Local delete of b whose mark was "dropped" -> reconcile flag set.
        (root / "b").unlink()
    ...
    def test_full_scan_reconciles_dropped_blob_delete(self, fake_redis, tmp_path):
        f = tmp_path / "o.conf"
        f.write_bytes(b"X")
    ...
    def test_reconcile_keeps_flag_until_redis_recovers(self, tmp_path):
        f = tmp_path / "o.conf"
        f.write_bytes(b"X")
        client = FlakyRedis()
        ...
        # Delete locally while Redis rejects deletes -> flag stays set, key kept.
        f.unlink()
        ...
        # Redis recovers -> next scan reconciles and clears the flag.
...
class TestDrainFailureFlagsReconcile:
    def test_failed_event_delete_sets_reconcile_flag(self, tmp_path):
        ...
        # Delete failed during the "outage" -> not lost, flagged for reconcile.
        assert m._delete_reconcile_needed is True
        assert client.get("ufm:state:o") == b"X"
